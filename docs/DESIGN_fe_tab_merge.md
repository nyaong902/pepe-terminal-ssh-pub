...
## 버그 수정 #11 — 캐시된 목록을 지워서 재로딩이 보이던 문제 (2026-07-30)

사용자 재현: "창1 → 창2로 분리할 때는 파일목록을 로딩하는 모습이 보이는데(문제), 창2 → 창1로
돌아갈 때(합쳐짐)는 바로 떠." — 이 **비대칭**이 결정적 단서였다.

버그 수정 #4(“연결 ID가 없습니다” 깜빡임 제거)에서 `loadDir` 에 넣은 lazy-remote 가드가
`setFiles([])` 로 **캐시된 목록까지 지우고** 있었다. 강등된(=재연결 대기) 탭은 `initialFiles`
(`tab.entries`)로 캐시를 이미 그려둔 상태인데 이게 비워지면서 `files.length === 0` → "로딩 중"
표시가 노출됐다. 즉 재로딩 자체보다 **캐시를 버린 것**이 눈에 보이는 원인이었다.
비대칭도 이걸로 설명된다 — 병합 방향은 대상 FileExplorer 가 이미 마운트돼 목록을 들고 있어서
가드를 타지 않는다.

- [FilePanel.tsx](../src/components/FilePanel.tsx) — lazy-remote 가드에서 `setFiles([])` 제거.
